...
- `mlfinlab.egg-info/` (build artifact)

If these reappear after a sync, delete them again. All configuration is in `pyproject.toml`.

### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `mlfinlab/` -> `src/mlfinlab/`
- **Import unchanged**: `import mlfinlab` still works

If upstream adds files to the old location, move them to `src/mlfinlab/`.
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/mlfinlab/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import mlfinlab" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                    # Install all deps
uv run python -c "import mlfinlab"         # Verify import
